...
## [2024-06] CTA 섹션 스타일 개선

### 완료된 일
- CTA 섹션 배경색 및 텍스트 스타일 개선
  - 배경색을 파란색(var(--primary-color))에서 흰색으로 변경
  - 텍스트 색상을 기본 텍스트 색상(var(--text-color))으로 변경
  - 부제목 텍스트 색상을 연한 텍스트 색상(var(--light-text))으로 변경
  - opacity 값 1로 조정하여 완전 불투명하게 수정
- CTA 섹션 버튼 스타일 개선
  - 첫 번째 버튼("Blog") 스타일 유지: 파란색 배경, 흰색 글씨 (.btn 클래스)
  - 두 번째 버튼("AIRTT") 스타일 수정: .btn-secondary에서 .btn-outline으로 변경하여 흰색 배경, 파란색 테두리, 파란색 글씨로 통일
  - 히어로 섹션과 CTA 섹션의 버튼 스타일 일관성 확보
- 섹션별 커스텀 버튼 스타일 제거
  - .cta .btn에 적용되었던 특별한 스타일 제거하여 전역 버튼 스타일만 사용하도록 수정
  - 버튼 스타일의 글로벌 일관성 유지

### 해야할 일
- 다양한 화면 크기에서 CTA 섹션 레이아웃 확인
- 다른 페이지에도 동일한 스타일 적용 확인 